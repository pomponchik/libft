...
/*   Updated: 2019/05/09 21:37:33 by ahalmon-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_bits_shift_left(void *mem, size_t size_bytes, size_t shift_bits)
{
	...
	{
		ft_bits_write_bit(ft_jump_pointer_size_t(mem, ((index + shift_bits + 1) \
		/ 8)), ((index + shift_bits) - ((((index + shift_bits) + 1) / 8) * 8) \
		- 1), ft_bits_read_bit(ft_jump_pointer_size_t(mem, ((index + 1) / 8)), \
		(index - (((index + 1) / 8) * 8) - 1)));
		index++;
	}
